...
public class FollowerInfoAction extends ActionSupport implements SessionAware{
	...
	public String execute(){
		 List<FollowListDTO> followerList = ( List<FollowListDTO>) session.get("followList");
		 System.out.println(flCheckNum);
		 session.put("searchFriendName", followerList.get(flCheckNum).getFollowUserName().toString());
		 session.put("searchFriendId", followerList.get(flCheckNum).getFollowUserId().toString());

		friendReadDTOList = friendReadDAO.getFriendRead(followerList.get(flCheckNum).getFollowUserId().toString());
		...
		try {
			...
			ps.setString(2, followerList.get(flCheckNum).getFollowUserId().toString());
