				...
				<s:form action="MyCheckAction">
					...
					<tbody>
						...
						<s:if test="session.alReadCheck == 'off'">
							<a href='<s:url action="MyAlReadAction" />' id="read" >読んだ!</a>
						</s:if>
						<s:if test="session.alReadCheck == 'on'">
							<a href='<s:url action="MyOffReadAction" />' id="read-off" >読んだ!を解除</a>
						</s:if>
						<a href='<s:url action="MyPageAction" />' id="check-btn" >戻る</a>
					</td>
				</tr>
				</tbody>
			</table>
