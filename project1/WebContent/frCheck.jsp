				...
				<s:form action="FrCheckAction">
					...
					<tbody>
						...
						<s:if test="session.alReadCheck == 'off'">
							<a href='<s:url action="FrAlReadAction" />' id="read" >読んだ!</a>
						</s:if>
						<s:if test="session.alReadCheck == 'on'">
							<a href='<s:url action="FrOffReadAction" />' id="read-off" >読んだ!を解除</a>
						</s:if>
						<a href='<s:url action="BackFollowersInfoAction" />' id="check-btn" >戻る</a>
					</td>
				</tr>
				</tbody>
			</table>
