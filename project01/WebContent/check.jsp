				...
				<s:form action="CheckAction">
					...
					<tbody>
						...
						<s:if test="session.alReadCheck == 'off'">
							<a href='<s:url action="AlReadAction" />' id="read" >読んだ!</a>
						</s:if>
						<s:if test="session.alReadCheck == 'on'">
							<a href='<s:url action="OffReadAction" />' id="read-off" >読んだ!を解除</a>
						</s:if>
						<a href='<s:url action="GoHomeAction" />' id="check-btn" >戻る</a>
					</td>
				</tr>
				</tbody>
			</table>
